...
async function measurePing(url: string): Promise<number> {
    ...
    try {
        ...
        // If a server cannot be reached, treat its ping as infinitely large
        return Infinity;
    }
}
...
async function assignClosestHealthyServer(userId: string): Promise<string> {
    ...
    console.log(`Assigned user ${userId} to server ${bestServer.server_url} (ping: ${minPing.toFixed(2)} ms)`);
