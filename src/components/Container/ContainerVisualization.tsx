...
export default function ContainerVisualization({
  ...
  const processedDataQuery = useMemo(() => {
    ...
    conditions.push(
      gte(processed_kraken_uniq_report.percentage, minReadPercentage)
    );
    ...
    return drizzleDB
      ...
      .orderBy(asc(sample_group_metadata.collection_date));
  }, [
    drizzleDB,
    selectedRank,
    startDate,
  ...
  const { data: processedData = [] } = useQuery(
    toCompilableQuery(processedDataQuery)
  );
  ...
  const closeData = useMemo(() => {
    const recs = dataWithProximity.filter(
      (r) => r?.proximity_group === 'Close'
    );
  ...
  return (
    <Dialog open={open} onOpenChange={(value) => !value && onClose()}>
      <DialogContent
        className={'max-w-full flex flex-col h-full overflow-y-scroll'}
      >
        <DialogTitle>
