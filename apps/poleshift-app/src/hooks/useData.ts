...
export const useData = () => {
  ...
  const penguinDataQuery = drizzleDB
    .select()
  ...
  const fileNodes = useMemo(
    () => arrayToRecord(fileNodesArray),
    [fileNodesArray]
  );
  const sampleGroups = useMemo(
    () => arrayToRecord(sampleGroupsArray),
    [sampleGroupsArray]
  );
  ...
  const updateFileNode = useCallback(
    async (id: string, updates: Partial<FileNodeWithChildren>) => {
  ...
  const deleteNode = useCallback(
    async (id: string) => {
      ...
      if (
        nodeResult.type === FileNodeType.Folder &&
        nodeResult.sampleGroupId === null
      ) {
  ...
  const moveNode = useCallback(
    async (nodeId: string, newParentId: string | null) => {
      try {
        ...
        setError(
          err instanceof Error ? err.message : 'Failed to move the node'
        );
        throw err;
      }
    },
  ...
  const getNodeChildren = useCallback(
    (nodeId: string) => {
      return Object.values(fileNodes).filter(
