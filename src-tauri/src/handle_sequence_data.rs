...
pub async fn handle_sequence_data<R: Runtime>(
    ...
    if !kudb_dir.exists() {
        println!("kudb directory does not exist, attempting to extract from '{}'", kudb_tar_xz.display());
    ...
    for original_path in &file_paths {
        ...
        println!("Copying '{}' to '{}'", original_path_buf.display(), temp_file_path.display());
        tokio::fs::copy(&original_path_buf, &temp_file_path).await.map_err(|e| {
    ...
    if !report_file_path.exists() {
        println!("Report file not found at expected location: {}", report_file_path.display());
    ...
    let report_content = tokio::fs::read_to_string(&report_file_path).await.map_err(|e| {
        println!("Failed to read report file '{}': {}", report_file_path.display(), e);
        PoleshiftError::IoError(e.to_string())
    })?;

    println!("Report file read successfully, content length: {}", report_content.len());

    emit_progress(&window, 100, "Complete")?;
    println!("Progress emitted: 100%, 'Complete'");

    println!("Cleaning up temporary input directory: {:?}", input_temp_dir);
    let _ = tokio::fs::remove_dir_all(&input_temp_dir).await.map_err(|e| {
        println!("Failed to remove temp directory: {}", e);
        e
    });

    let raw_files: Vec<FileMeta> = file_paths.iter().map(|f| {
