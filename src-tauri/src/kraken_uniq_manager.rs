...
const DB_DOWNLOAD_URL: &str = "https://poleshift.icarai.cloud/storage/v1/object/public/kraken-uniq-dist/krakenuniq-ubuntu-dist.tar.gz?t=2024-12-02T22%3A59%3A42.547Z";

pub struct KrakenManager;
...
impl KrakenManager {
    pub async fn ensure_krakenuniq<R: Runtime>(app_handle: &tauri::AppHandle<R>) -> Result<PathBuf, String> {
        ...
        if !Self::verify_krakenuniq(&db_path) {
            Self::emit_status(app_handle, "Kraken Uniq not found, starting download...")?;
            Self::download_and_extract_krakenuniq(db_path.clone(), app_handle).await?;
        }

        Ok(db_path)
    }

    fn verify_krakenuniq(path: &PathBuf) -> bool {
        if !path.exists() {
            return false;
        }
    ...
    async fn download_and_extract_krakenuniq<R: Runtime>(db_path: PathBuf, app_handle: &tauri::AppHandle<R>) -> Result<(), String> {
        ...
        Self::emit_status(app_handle, "Downloading krakenuniq...")?;

        // Download the file
        let response = reqwest::get(DB_DOWNLOAD_URL)
            .await
            .map_err(|e| format!("Failed to download krakenuniq: {}", e))?;
        ...
        Self::emit_status(app_handle, "Extracting krakenuniq...")?;
        ...
        Self::emit_status(app_handle, "Krakenuniq setup complete")?;
